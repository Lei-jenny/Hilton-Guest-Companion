...
const DashboardStep: React.FC<DashboardStepProps> = ({ session }) => {
  ...
  useEffect(() => {
    ...
    // 2. Load Attractions (Mock only; AI generation is user-triggered)
    ...
    // 3. Itinerary is generated on demand to avoid automatic model calls
    setItinerary('');
  ...
  const handleGenerateRecommendations = async () => {
    setLoadingAttractions(true);
    const dynamicList = await generateDynamicAttractions(session.booking.location, session.travelStyle, true);
  ...
  const handleGenerateItinerary = async () => {
    ...
    const text = await generateItinerary(session.booking, session.travelStyle, true);
    setItinerary(text);
    setLoadingItinerary(false);
  };
